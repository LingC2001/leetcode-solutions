...
def generate_combined_difficulty_ring_svg(counts):
  ...
  <text x="100" y="115" text-anchor="middle" font-family="Arial, sans-serif" font-size="14" class="text-secondary">Problems</text>
    ...
    (counts.get("easy", 0) / total) * 360
    (counts.get("medium", 0) / total) * 360
    (counts.get("hard", 0) / total) * 360
...
def main():
    ...
    totals = {"easy": 600, "medium": 1300, "hard": 500}
    ...
    with open(assets_dir / "dashboard_stats.json", "w", encoding="utf-8") as f:
        json.dump(stats, f, indent=2)

    print("📈 Dashboard generation complete!")
    print(
        f"📊 Summary: {total_problems} problems ({counts['easy']} easy, {counts['medium']} medium, {counts['hard']} hard)"
    )
    print(f"🌟 Topics covered: {len([t for t, c in topic_counts.items() if c > 0])}")
    print(
        f"💻 Languages: Python({language_counts['Python']}), C++({language_counts['C++']}), Java({language_counts['Java']})"
    )
    print(
        f"✅ Status: {status_counts.get('Complete', 0)} complete, {status_counts.get('Solutions Only', 0)} solutions only"
    )
